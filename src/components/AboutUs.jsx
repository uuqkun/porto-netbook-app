    ...
    <section className={`flex md:flex-row flex-col py-10`}>
      ...
      <div className={`flex-1 ${layout.flexStart} flex-col lg:mb-0 mb-10`}>
        ...
        <div>
          {joinItems.map((item, index) => (
            <li
              className={`${
                index !== joinItems.length - 1 ? "mb-2" : "mb-0"
              } list-none flex items-center font-quicksand font-semibold text-heading`}
              key={item.id}
            >
      ...
      <div className={`flex-1 flex ${layout.flexCenter} gap-7`}>
        <img
          src={discussion}
          alt="discussion"
          className="w-1/2 flex-1 lg:w-[270px] lg:h-[378px] rounded-xl shadow-sm"
        />
        <div className="w-1/2 flex flex-col gap-7 flex-1">
          <img
            src={message}
            alt="message"
            className="w-full lg:w-[265px] lg:h-[238px] rounded-xl shadow-sm"
          />
          <img
            src={network}
            alt="network"
