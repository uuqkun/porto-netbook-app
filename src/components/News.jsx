...
const News = () => {
  return (
    <section className="flex flex-col py-20">
      ...
      <div className="flex md:overflow-auto overflow-x-scroll whitespace-nowrap">
        {blogs.map(({ id, title, image, link }, index) => (
          <a
            href={link}
            key={id}
            className={`${
              index !== blogs.length - 1 ? "mr-16" : "mr-0"
            } md:max-w-[340px] max-w-[260px] min-w-[220px] whitespace-normal`}
          >
            {/* blog image */}
            ...
            <div className="w-full flex items-center p-2">
              ...
              <div className="flex-1 flex flex-col">
                <h4
                  className={`sm:${font.heading3} text-[16px] font-quicksand text-heading font-semibold md:mb-5 mb-3`}
                >
                  {title}
                </h4>
                <p
                  className={`flex gap-4 items-center ${font.paragraph} hover:text-primary active:text-primary`}
                >
                  Continue Reading
                  <img
                    src={blackArrowRight}
